...
1. **Discover chain** (optional) — extend a partial chain by appending missing intermediates.
2. **Determine requirements** — how many SCTs and distinct operators are needed.
3. **Find compatible logs** — filter the log list by temporal compatibility and chain validation.
4. **Devise a strategy** — rank and bucket the compatible logs.
5. **Execute submissions** — submit to logs concurrently, collect responses, and evaluate quorum.

## 1. Chain Discovery (`pki/discovery.go`)

When `discoverChain` is true, ctsubmit attempts to extend the submitted certificate chain before log selection begins. This is useful when a caller submits only a leaf certificate (or a partial chain) without knowing which intermediate CA certificates to include.

Discovery uses a precomputed `optimal_parents.csv` file (generated by `cmd/optimalparents`) that maps each Authority Key Identifier (AKI) to the SHA-256 hash of the optimal parent certificate for each log list context (Usable TLS, Active TLS, Test TLS, Usable BIMI). The optimal parent is the certificate whose chain is acceptable to the most CT logs, with tiebreaker logic preferring shorter chains, then later expiry dates, then lexicographically earlier certificate hashes.

The discovery loop:

1. Parse the last certificate in the chain and extract its AKI.
2. Look up the AKI in the optimal parents map for the applicable log list context.
3. If the looked-up parent is a self-signed root, stop — roots are not needed in CT submissions.
4. Otherwise, retrieve the parent's DER from CCADB data, append it to the chain, and repeat from step 1.
...
## 3. Compatible Log Selection (`compatibleLogs.go`)
...
## 5. Submission Execution (`orchestrate.go`)
