...
The WISeKey improved version supports loading additional Static CT test logs from test-service configuration. These logs are added only to the test TLS log list and are used only by requests that set `testLogs: true`.

WISeKey has two minimal Sunlight test CT logs:

| Operator | Log | Submission URL | Monitoring URL |
|---|---|---|---|
| Contoso | Cedar | `https://cedar.sunlight.contoso.wisekeydev.com/` | `https://cedar.skylight.contoso.wisekeydev.com/` |
| Fabrikam | Topaz | `https://topaz.sunlight.fabrikam.wisekeydev.com/` | `https://topaz.skylight.fabrikam.wisekeydev.com/` |

Both logs accept the same two WISeKey demo roots:

```text
OISTE WISeKey Demo Root CA 1 (RSA 2048)
WISeKey DEMO Root CA (ECDSA P-384)
```

#### Local Config Layout

For a local Docker test, prepare a directory like this:

```text
ctsubmit-test-config/
  config.yaml
  static-logs/
    contoso-cedar-public-key.der
    fabrikam-topaz-public-key.der
    wisekey-demo-roots/
      oiste-wisekey-demo-root-ca-1.pem
      wisekey-demo-root-ca-ecdsa-p384.pem
```

Fetch the Static CT log public keys from each log's `log.v3.json`:

```bash
mkdir -p ctsubmit-test-config/static-logs/wisekey-demo-roots

curl -fsS https://cedar.sunlight.contoso.wisekeydev.com/log.v3.json \
  | jq -r '.key' \
  | base64 -d \
  > ctsubmit-test-config/static-logs/contoso-cedar-public-key.der

curl -fsS https://topaz.sunlight.fabrikam.wisekeydev.com/log.v3.json \
  | jq -r '.key' \
  | base64 -d \
  > ctsubmit-test-config/static-logs/fabrikam-topaz-public-key.der
```

Place the two WISeKey demo root certificates in:

```text
ctsubmit-test-config/static-logs/wisekey-demo-roots/
```

The files may be PEM or DER. The same root directory can be reused by both logs.

#### `config.yaml`

Create `ctsubmit-test-config/config.yaml`:
...
customStaticTestLogs:
  ...
  logs:
    - operator: "Contoso"
      name: "Contoso Cedar"
      submissionURL: "https://cedar.sunlight.contoso.wisekeydev.com/"
      monitoringURL: "https://cedar.skylight.contoso.wisekeydev.com/"
      checkpointOrigin: "cedar.sunlight.contoso.wisekeydev.com"
      mmd: 60
      publicKeyFile: "/config/static-logs/contoso-cedar-public-key.der"
      acceptedRootsDir: "/config/static-logs/wisekey-demo-roots/"

    - operator: "Fabrikam"
      name: "Fabrikam Topaz"
      submissionURL: "https://topaz.sunlight.fabrikam.wisekeydev.com/"
      monitoringURL: "https://topaz.skylight.fabrikam.wisekeydev.com/"
      checkpointOrigin: "topaz.sunlight.fabrikam.wisekeydev.com"
      mmd: 60
      publicKeyFile: "/config/static-logs/fabrikam-topaz-public-key.der"
      acceptedRootsDir: "/config/static-logs/wisekey-demo-roots/"
```

The `checkpointOrigin` values must match the first line of each Static CT checkpoint.

#### Run Locally

Build the WISeKey improved image:

```bash
git switch all-improvements
docker build -t ctsubmit:wisekey-improved .
```

Run the local test container with the config mounted at `/config`:

```bash
docker run --rm \
  -p 8080:8080 \
  -p 8081:8081 \
  -v "$(pwd)/ctsubmit-test-config:/config:ro" \
  ctsubmit:wisekey-improved
```

Confirm the logs are loaded:

```bash
curl -fsS http://localhost:8080/test_tls_logs.json
curl -fsS http://localhost:8080/dashboard?loglist=testtls
curl -fsS http://localhost:8081/readyz
```

Submit test chains with `testLogs: true`. Because Contoso and Fabrikam are configured as two different operators, quorum can be tested with two SCTs from two operators:
...
For early smoke tests, use `operators: 1` and `scts: 1`.

Operational recommendation:

1. Use this configuration only in test deployments.
2. Keep internal Static CT log configuration out of production deployments.
3. Use `verbose: true` during validation to confirm Contoso and Fabrikam appear in the strategy.
4. After validation, deploy the same configuration to a dedicated internal test service, for example `ctsubmit-internal-test.<domain>`.

Implementation details are documented separately in [ctsubmit-custom-static-test-logs.md](ctsubmit-custom-static-test-logs.md).
...
Use it for public test CT logs with `testLogs: true`, and require clients to explicitly choose test behavior in the request. For internal Static CT log testing, either:

- keep one test service and enable `customStaticTestLogs`, or
